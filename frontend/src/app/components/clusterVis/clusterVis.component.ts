...
@Component({
  selector: 'app-clusterVis',
  templateUrl: './clusterVis.component.html',
  styleUrls: ['./clusterVis.component.css']
})
export class ClusterVisComponent implements OnInit {
  // Loading indicator
  ...
  clusters: Artist[][] = [];
  intraCommunityEdges: exhibited_with[][] =[];
  interCommunityEdges: exhibited_with[] =[];
...
private  loadInitialData() {
  ...
  // Append a single SVG container for all sunburst charts and network visualization
const svgContainer = d3.select("#cluster-container")
.append("svg")
.attr("width", this.width + (this.margin * 2))
.attr("height", this.height + (this.margin * 2));

// Append a group element for positioning sunburst charts
const sunburstGroup = svgContainer.append("g")
.attr("transform", `translate(${this.width / 2 + this.margin}, ${this.height / 2 + this.margin})`);

// Append a group element for positioning network visualization
const networkGroup = svgContainer.append("g")
.attr("transform", `translate(${this.width / 2 + this.margin}, ${this.height / 2 + this.margin})`);

  this.artistService.clusterAmountArtistsNationality([300,400],2).subscribe(data => {
    this.clusters = data[0];
    this.intraCommunityEdges = data[1];
    this.interCommunityEdges = data[2];
    this.createGlobalColorScale('nationality'); // Setup the color scale after data is loaded
    //TO DO!!!!!!
    this.selectionService.selectArtist(this.clusters[0]);
    this.createGlobalColorScale('nationality');
    this.clusters.forEach((cluster, index) => {
      this.createSvgForCluster('nationality',cluster, this.intraCommunityEdges[index]);
    });
...
private createSvgForCluster(value:string,artists: Artist[], intraCommunityEdges: exhibited_with[]): void {
  ...
  const countryCentroids = this.drawSunburst(value,artists, intraCommunityEdges,svg);
  this.drawNetwork(value, countryCentroids, artists, intraCommunityEdges,svg);
...
updateCluster(k: number) {
  ...
  this.artistService.clusterAmountArtistsNationality(range[0], k).subscribe((data) => {
    ...
    this.renderClusters(this.clusters,this.intraCommunityEdges, this.interCommunityEdges)
  }, (error) => {
    console.error('There was an error', error);
  });
  }
...
updateVisualization(type: string, value: string) {
  ...
  if (this.svg) {
    ...
    if (type === 'sunburst') {
      ...
      this.updateNodeSize(this.decisionService.getDecisionSize());
...
private getNodeSize():number[]{
  // Select all circles with the class 'node'
  const circles = container.selectAll('.node');
...
private drawSunburst(value: string, artists: Artist[], intraCommunityEdges:exhibited_with[],container: d3.Selection<d3.BaseType, unknown, HTMLElement, any>):
{ [country: string]: { startAngle: number, endAngle: number, middleAngle: number, color: string | number, country: string} }{


  // Get the width and height of the container
  const width = (container?.node() as Element)?.clientWidth || 0;
  const height = (container?.node() as Element)?.clientHeight || 0;

  // Now you can use width and height as needed in your function

  const outerRadius = Math.min(width, height) / 2;
  const sunburstThickness = 50;
  const innerRadius = outerRadius - sunburstThickness;

  const countryMap = new Map<string, Artist[]>();
  if(value === 'nationality'){
  const sortedArtists:Artist[] = this.prepareData('nationality', artists);
  sortedArtists.forEach(artist => {
    ...
    const sortedArtists:Artist[] = this.prepareData('birthcountry', artists);
    sortedArtists.forEach(artist => {
    ...
    const sortedArtists:Artist[] = this.prepareData('deathcountry', artists);
    sortedArtists.forEach(artist => {
    ...
    const sortedArtists:Artist[] = this.prepareData('mostexhibited', artists);
    sortedArtists.forEach(artist => {
  ...
  const data = countries.map((country, i) => {
    ...
    return {
      ...
      color: this.globalColorScale(country)  // Assign color by index
    };
  });
...
private createGlobalColorScale(value: string): void {
  ...
  this.globalColorScale = this.createColorScale(countryArray);
}




private drawNetwork(value: string, 
  countryCentroids: { [country: string]: { startAngle: number, endAngle: number, middleAngle: number, color: string | number, country: string} },
  artists: Artist[], 
  intraCommunityEdges:exhibited_with[],
  container: d3.Selection<d3.BaseType, unknown, HTMLElement, any>): void {


    const degrees = this.calculateNodeDegrees(intraCommunityEdges);
    const normalizedDegrees = this.normalizeSqrt(degrees);
    const degreesMap = normalizedDegrees;
 
 
let nodes: ArtistNode[] = [];
if(value === 'nationality'){
  // Format artists as nodes
  nodes = artists.map((artist: Artist) => {
    const countryData = countryCentroids[artist.nationality];
    const degree = degreesMap.get(artist.id) || 0;
...
const sharedExhibitionMinArtworksValues = intraCommunityEdges.map((relationship: exhibited_with) => relationship.sharedExhibitionMinArtworks);
...
const formattedRelationships = intraCommunityEdges.map((relationship: exhibited_with, index) => {
...
this.edges = edges;
  ...
  const circles = container.selectAll('.node')
    ...
    .append('circle')
    .attr('class', 'node')
    .attr('r', (d: any) => this.calculateRadiusForNode(degreesMap.get(d.id) || 0)) // Adjust radius based on degree
  ...
  text.append("text")
    ...
    const sizes = this.getNodeSize(nodes);
...
private prepareData(value:string, artists:Artist[]): any[] {
